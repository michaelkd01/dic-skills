...
description: Assess a piece of content ... pull it in, summarise it, give an honest quality read, surface themes relevant to Michael's ventures and interests, and end with an adopt/try/skip call. Source-agnostic successor to video-insights. VIDEO sources auto-trigger: use this skill EVERY time a video link or uploaded video appears (youtube.com/watch, youtu.be, youtube.com/shorts, youtube.com/live, m.youtube.com; or x.com/twitter.com, vimeo.com, tiktok.com, facebook.com, reddit, a direct .mp4/.m3u8/.webm URL, or an uploaded video file). YouTube transcripts come from the TranscriptAPI connector; other video is transcribed in a code-execution sandbox (yt-dlp + Whisper). TEXT sources (an email, a newsletter, an article URL, or pasted/screenshotted copy) trigger ONLY when the user explicitly asks or shares the content as the subject of their message ... never auto-scan the inbox or fire on an email that is merely present in context. Also trigger on phrases like "summarise this", "what's this about", "is this worth reading or watching", "assess this email", "key points from this", "give me the insights on this". Output is inline only and proposes actions ... it never writes to Notion, Linear, Obsidian, or Todoist.
---
